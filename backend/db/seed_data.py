"""
LexPort — Database Seed Data & Real-World Case Presets
Seeds 5 documented cross-border failure cases representing Layer 1b classification mismatches,
banned claims, product bans, and hazmat shipping escalations.
"""
...
from backend.db.models_db import User, Listing, Inspection, ComplianceResultRecord, AuditHashBlock
...
async def seed_database(session: AsyncSession) -> None:
    """Populates the database with initial users and preset case studies."""
    existing_user = await session.execute(select(User).limit(1))
    if existing_user.scalars().first() is not None:
        return

    logger.info("[SEED] Seeding initial users and preset listings...")
    ...
    # Seed preset listings
    for item in PRESET_LISTINGS:
        listing = Listing(
            id=item["id"],
            title=item["title"],
    ...
    logger.info(f"[SEED] Successfully seeded {len(PRESET_LISTINGS)} preset listings.")
